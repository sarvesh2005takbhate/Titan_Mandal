...
from src.advanced_analysis import (
    ...
    modularity_null_model, negative_edges, parallel_analysis, principal_components, respondent_k_sensitivity,
    rewired_null, statement_robust_cores, statement_threshold_sensitivity,
)
...
def main():
    ...
    add(f"- **No distinct opinion camps.** Respondent communities (Q = {Q:.3f}) are {'not significantly' if not significant else 'significantly'} stronger than in shuffled data "
        f"(null Q = {null['null_mean']:.3f} ± {null['null_std']:.3f}, p = {null['p']:.2f}), change between runs, and the main opinion scores are unimodal. Opinions vary along a continuum, not in blocs.")
    add(f"- **A leading axis of disagreement:** regulation and traditional structure ({', '.join(pc2_pos)}) versus open, flexible learning ({', '.join(pc2_neg)}). "
        f"All {len(neg)} negative statement correlations involve Education{' and their bootstrap 95% intervals exclude zero' if neg_ci_ok else ''}.")
    add(f"- **Domains matter, but themes cross them.** Within-domain links are {enrich['within_enrichment']:.1f}× more common than chance, yet {pct(enrich['cross_share'])} of links cross domains "
    ...
    add(f"| Clustering (baseline) | {stats1['clustering_unweighted']:.2f} (shuffled {null['null_clustering_mean']:.2f}) | {stats2['clustering_unweighted']:.2f} (rewired {rewired['null_clustering']:.2f}) |")
    ...
    add(f"- **A continuum, not camps:** respondents' PC2 scores (Section 3.5) are unimodal (bimodality coefficient {pc2_bimodality:.2f}; > 0.555 would suggest two camps). "
        ...
        f"Communities are slices of this continuum: they split along PC2 (η² = {pc2_eta:.2f}; C{open_comm} {pc2_by_comm[open_comm]:+.2f} vs C{control_comm} {pc2_by_comm[control_comm]:+.2f}), only modestly more than cutting shuffled data would (η² = {null['null_pc2_eta_mean']:.2f}).")
    ...
    add(f"- **Domains vs themes:** if links ignored domains, {pct(enrich['expected_cross_share'])} would cross domains; only {pct(enrich['cross_share'])} do, so within-domain links are {enrich['within_enrichment']:.1f}× over-represented. Domains matter, but most links still cross them.")
    add(f"- **Clusters:** the positive network is more modular than degree-preserving rewired graphs (Q = {rewired['q_unweighted']:.2f} vs {rewired['null_q_mean']:.2f} ± {rewired['null_q_std']:.2f}, z = {rewired_q_z:.0f}), "
        ...
        f"We therefore report **robust cores**: groups linked by statement pairs that cluster together in ≥ {pct(cores['min_coassign'])} of {cores['runs']} bootstrap resamples. "
        f"The last column averages over all pairs in a core; a typical pair of statements clusters together in only {pct(cores['overall_coassign'])}.")
    add("")
    ...
    add(f"- **Unconnected statements:** {', '.join(signed_isolates)} have no edge at all. "
        + (f"{', '.join(c + ' (' + pct(summary.loc[c, 'neutral']) + ' Neutral)' for c in uncertain)} are among the five most-Neutral statements: uncertainty, not conviction, dominates them. " if uncertain else "")
    ...
    add(f"- **PCA with parallel analysis:** {parallel['n_above_noise']} components exceed the 95th percentile of shuffled data. PC1 ({pct(pca['explained'][0], 1)}) is general agreement ({pct(pca['pc1_same_sign_share'])} of loadings share a sign; r = {pca['pc1_vs_mean_answer_r']:.2f} with mean answer). "
        ...
        f"It is only slightly larger than PC3 ({pct(pca['explained'][2], 1)}), so disagreement is multi-dimensional; we interpret PC2 because independent evidence agrees with it"
    ...
    add(f"- **Threshold:** edges drop from {int(thr_sens['edges'].iloc[0])} at 0.20 (up to ~{thr_sens['expected_false_edges'].iloc[0]:.0f} chance edges) to {int(thr_sens['edges'].iloc[-1])} at 0.50. "
        f"{CORR_THRESHOLD:.2f} keeps most statements connected while at most ~{thr_row['expected_false_edges'] / thr_row['edges']:.0%} of edges could be chance.")
    ...
    add(f"- **Statement-level results hold:** negative edges have bootstrap intervals excluding zero, Spearman gives nearly the same correlations (r = {ev['pearson_spearman_r']:.2f}), and each core in Section 3.3 is co-clustered in more than half of 200 bootstrap resamples.")
    add("")
    ...
    add(f"3. **The network shows a continuum, not camps.** Communities are no stronger than in shuffled data, are unstable, and PC2 scores are unimodal. Respondents are embedded among neighbours with similar views (assortativity {assort_pc2:.2f}), but these neighbourhoods blend into each other rather than forming blocs. Averages alone could not show this; the null model makes it explicit.")
    add(f"4. **Domains shape opinions but do not contain them.** Within-domain links are {enrich['within_enrichment']:.1f}× chance, yet most links cross domains and {len(mixed_cores)} of {len(cores['cores'])} reproducible clusters mix them"
        + (f" (regulation and rights: {', '.join(privacy_core['members'])})." if privacy_core else "."))
    add(f"5. **Uncertain topics stay disconnected.** The most-Neutral statements ({', '.join(uncertain)}) correlate with nothing, suggesting unformed rather than divided opinions.")
    add("")
    add("### Limitations")
    add("")
    ...
    add(f"- {n_tests:,} pairwise tests: a few statement edges may be chance; only the strongest survive Bonferroni.")
    ...
    add("The survey describes a broadly like-minded class whose disagreement is spread thinly over several dimensions, the largest contrasting structure and regulation with openness and flexibility, mostly through Education. "
        "Network analysis reveals robust cross-domain themes and genuine opposing pairs of statements. Null models and bootstraps show where the network is informative (edges, local neighbourhoods, statement cores) and where it is not (respondent communities).")
    add("")
